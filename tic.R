...
if (!ci_has_env("PARAMTEST")) {
  ...
  get_stage("install") %>%
    ...
    add_code_step(system("Rscript -e 'print(paste0(getwd(), \"/lightgbm\")); setwd(paste0(getwd(), \"/lightgbm\")); system(\"Rscript build_r.R\")'"))
  ...
  get_stage("install") %>%
    add_step(step_install_deps()) %>%
    add_code_step(git2r::clone("https://github.com/microsoft/LightGBM", "lightgbm")) %>%
    add_code_step(system("Rscript -e 'setwd(paste0(getwd(), \"/lightgbm\")); system(\"Rscript build_r.R\")'"))
